...
///
/// The `+1` over `SESSION_POOL_SIZE` in `BATCH_QUEUE_CAPACITY` prevents the
/// deadlock described in REWORK.md §Batch queue capacity.
...
// SAFETY: the queue is SPSC; only one thread calls push and one calls pop.
...
impl BatchQueue {
    ...
    ///
    /// Called by SubmissionConsumer while holding the poll guard.  Spins are
    /// expected to be brief (CompletionConsumer pops quickly on GPU completion).
    pub fn push(&self, entry: BatchEntry) {
        loop {
            ...
            if tail.wrapping_sub(head) < self.capacity {
                ...
                // SAFETY: producer has exclusive access to this slot (tail not yet visible to consumer).
    ...
    pub fn pop(&self) -> Option<BatchEntry> {
        ...
        // SAFETY: consumer has exclusive access once head < tail.
...
impl Drop for BatchQueue {
    fn drop(&mut self) {
        // Drain any remaining (unconsumed) entries to call their destructors.
